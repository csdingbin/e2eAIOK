...
from ZeroShotProxy import compute_zen_score, compute_denas_jc_score, compute_denas_score, compute_test_score, compute_zen_syncflow_score, compute_zen_syncflow_bn_score, compute_fast_score, compute_te_nas_score, compute_syncflow_score, compute_gradnorm_score, compute_NASWOT_score
...
def parse_cmd_options(argv):
    ...
    parser.add_argument('--belta', type=float, default=1.0,
                        help='hyperpara for latency score')
    ...
    try:
        if args.zero_shot_score == 'Zen':
            ...
            the_nas_core = compute_syncflow_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size)
        elif args.zero_shot_score == 'Zen_Syncflow':
            the_nas_core = compute_zen_syncflow_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size,
                                                                       mixup_gamma=args.gamma)
        elif args.zero_shot_score == 'DENAS':
            the_nas_core = compute_denas_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size,
                                                                       mixup_gamma=args.gamma)
        elif args.zero_shot_score == 'DENAS_JC':
            the_nas_core = compute_denas_jc_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size,
                                                                       mixup_gamma=args.gamma, belta=args.belta)
        elif args.zero_shot_score == 'TEST':
            the_nas_core = compute_test_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size,
                                                                        mixup_gamma=args.gamma)

        elif args.zero_shot_score == 'Zen_Syncflow_BN':
            the_nas_core_info = compute_zen_score.compute_nas_score(model=the_model, gpu=gpu,
                                                                    resolution=args.input_image_size,
                                                                    mixup_gamma=args.gamma, batch_size=args.batch_size,
                                                                    repeat=1)
            zen_nas_core = the_nas_core_info['avg_nas_score']

            syn_nas_core = compute_syncflow_score.do_compute_nas_score(model=the_model, gpu=gpu,
                                                                       resolution=args.input_image_size,
                                                                       batch_size=args.batch_size)
            the_nas_core = zen_nas_core + syn_nas_core
            ...
            the_nas_core = compute_NASWOT_score.compute_nas_score(gpu=gpu, model=the_model,
                                                                  resolution=args.input_image_size,
                                                                  batch_size=args.batch_size)
        elif args.zero_shot_score == 'Zen_NASWOT':
            the_nas_core_info = compute_zen_score.compute_nas_score(model=the_model, gpu=gpu,
                                                                    resolution=args.input_image_size,
                                                                    mixup_gamma=args.gamma, batch_size=args.batch_size,
                                                                    repeat=1)
            the_zen_nas_core = the_nas_core_info['avg_nas_score']

            the_NASWOT_core = compute_NASWOT_score.compute_nas_score(gpu=gpu, model=the_model,
                                                                  resolution=args.input_image_size,
                                                                  batch_size=args.batch_size)
            the_nas_core = 0.9 * the_zen_nas_core + 0.1 * the_NASWOT_core 

        elif args.zero_shot_score == 'Fast':
            the_nas_core_info = compute_fast_score.compute_nas_score(model=the_model, gpu=gpu,
                                                                    resolution=args.input_image_size,
                                                                    mixup_gamma=args.gamma, batch_size=args.batch_size,
                                                                    repeat=1)
            the_nas_core = the_nas_core_info['avg_nas_score']                                              
